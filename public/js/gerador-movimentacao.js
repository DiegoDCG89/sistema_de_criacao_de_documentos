...
async function processarFormulario(e) {
  ...
  try {
    ...
    dadosFormularioAtual = {
      ...
      temCarro, temMoto,
      carro: temCarro ? {
        marca: document.getElementById('carro_marca').value.trim() || 'XXXXXXX',
        placa: document.getElementById('carro_placa').value.trim().toUpperCase() || 'XXXXXXX',
        cor: document.getElementById('carro_cor').value.trim() || 'XXXXXXX',
        ano: document.getElementById('carro_ano').value.trim() || 'XXXXXXX'
      } : null,
      moto: temMoto ? {
        marca: document.getElementById('moto_marca').value.trim() || 'XXXXXXX',
        placa: document.getElementById('moto_placa').value.trim().toUpperCase() || 'XXXXXXX',
        cor: document.getElementById('moto_cor').value.trim() || 'XXXXXXX',
        ano: document.getElementById('moto_ano').value.trim() || 'XXXXXXX'
      } : null,
    ...
    if (dadosFormularioAtual.dependentes.length > 0) {
      ...
      rowsDep.push(gerarLinhaDep("XXXXXXX", "XXXXXXX", "XXXXXXX", "XXXXXXX"));
    }
    const tabelaDep = new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: rowsDep });

    // --- 1. DIEx OPÇÃO MOVIMENTAÇÃO ---
      ...
      sections: [{
        ...
        children: [
          new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: { after: 200 }, children: [
            new TextRun({ text: `Solicitou ao Chefe da Divisão Administrativa do Batalhão a Indenização de ${txtIndenizacoes.join(", ")}, em virtude de sua movimentação para o ${dadosFormularioAtual.omDestino}, ${dadosFormularioAtual.cidadeResidira}, conforme o publicado no ${dadosFormularioAtual.adtDcem}.` })
          ]}),
    ...
    const urlPdfDocs = await subirArquivoParaServidor(blobPdf, `Checklist_Movimentacao_${nomeBase}.pdf`);
    const urlDiex = await subirArquivoParaServidor(blobDIEx, `DIEx_Movimentacao_${nomeBase}.docx`);
    const urlNotaBar = await subirArquivoParaServidor(blobBAR, `NotaBAR_Movimentacao_${nomeBase}.docx`);
    const urlOrdPagto = await subirArquivoParaServidor(blobOrdPagto, `OrdPagto_Movimentacao_${nomeBase}.docx`);

    const payloadReq = {
      id: 'MOV-' + Date.now().toString().slice(-6),
      dataCriacao: new Date().toLocaleDateString('pt-BR'),
      modalidade: 'Movimentacao',
      omDestino: dadosFormularioAtual.omDestino,
