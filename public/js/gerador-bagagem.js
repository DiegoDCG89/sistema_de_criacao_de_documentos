  ...
  dependentes.forEach(dep => {
    container.innerHTML += `
      <div class="grid grid-cols-1 sm:grid-cols-12 gap-2 bg-stone-50 p-2.5 rounded border border-stone-200">
        <div class="sm:col-span-5"><input type="text" placeholder="Nome Completo" value="${dep.nome}" oninput="window.atualizarDep(${dep.id}, 'nome', this.value.toUpperCase())" class="w-full text-xs border border-stone-300 rounded px-2 py-1.5 uppercase"></div>
        ...
        <div class="sm:col-span-2"><input type="text" placeholder="Dt Nasc" value="${dep.dtNasc}" maxlength="10" oninput="window.aplicarMascaraDataDep(this, ${dep.id})" class="w-full text-xs border border-stone-300 rounded px-2 py-1.5"></div>
        <div class="sm:col-span-2 flex gap-1"><input type="number" placeholder="Idade" value="${dep.idade}" oninput="window.atualizarDep(${dep.id}, 'idade', this.value)" class="w-full text-xs border border-stone-300 rounded px-2 py-1.5"><button type="button" onclick="window.removerDependente(${dep.id})" class="text-red-500 hover:text-red-700 px-2 font-bold"><i data-lucide="trash-2" class="w-4 h-4"></i></button></div>
      </div>
    `;
  });
  if (window.lucide) lucide.createIcons();
...
async function subirArquivoParaServidor(blob, nomeArquivo, nomePasta) {
  const res = await fetch(`/api/upload?filename=${encodeURIComponent(nomeArquivo)}&folder=${encodeURIComponent(nomePasta)}`, {
    method: 'POST',
    body: blob
  });
...
async function processarFormulario(e) {
  ...
  try {
    ...
    const docDIEx = new Document({
      sections: [{
        ...
        children: [
          ...
          new Paragraph({ spacing: { before: 400, after: 400 }, children: [new TextRun({ text: "DIEx S/Nº", bold: true, size: 24 }), new TextRun({ text: `\t\t\t\tLorena - SP, ${obterDataExtenso()}.`, size: 24 })]}),
          new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: { after: 200 }, children: [new TextRun({ text: `Solicito o encaminhamento deste documento e anexo ao CMSE, a fim de que seja feito o repasse, para esta OM, dos recursos necessários para o pagamento de indenização de transporte de bagagem na mesma sede ao ${dadosFormularioAtual.posto} ${dadosFormularioAtual.nome}, conforme dados abaixo:`, size: 24 })]}),
    ...
    if (dadosFormularioAtual.dependentes.length > 0) textoDepBAR = dadosFormularioAtual.dependentes.map(d => `${d.nome} – ${d.parentesco}`).join('; ');

    const docBAR = new Document({
      sections: [{
        properties: { page: { margin: { top: 1134, right: 1134, bottom: 1134, left: 1134 } } },
        children: [
          new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: { after: 200 }, children: [new TextRun({ text: `Solicitou o pagamento de indenização de transporte de bagagem na mesma sede, em virtude da ocupação de PNR ocorrida em ${dadosFormularioAtual.dataOcupacao}, conforme publicado no ${dadosFormularioAtual.bi}.`, size: 24 })]}),
          ...
          new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: { after: 200 }, children: [new TextRun({ text: "Em consequência a divisão administrativa elabore o processo de pagamento da indenização de transporte de bagagem e demais interessados tomem as providências decorrentes.", size: 24 })]}),
          new Paragraph({ children: [new TextRun({ text: `(Nota p/ BAR nº - DA.5, de ${obterDataExtenso().split(' de ').slice(1).join(' de ')})`, size: 24 })]})
        ]
      }]
    });
    ...
    const nomePasta = `Bagagem - ${dadosFormularioAtual.posto} ${dadosFormularioAtual.nome}`;
    
    await subirArquivoParaServidor(blobDIEx, `DIEx_Bagagem_${nomeUnico}.docx`, nomePasta);
    await subirArquivoParaServidor(blobBAR, `Nota_BAR_Bagagem_${nomeUnico}.docx`, nomePasta);
    await subirArquivoParaServidor(blobPdf, `Checklist_Bagagem_${nomeUnico}.pdf`, nomePasta);

    // MÁGICA DO PROTOCOLO ADMIN (Sem Arquivos Pesados)
    const payloadReq = {
      id: 'BAG-' + Date.now().toString().slice(-6),
      dataCriacao: new Date().toLocaleDateString('pt-BR'),
      modalidade: 'Bagagem Mesma Sede',
    ...
    if (!resDb.ok) throw new Error('Não foi possível registrar o protocolo.');
